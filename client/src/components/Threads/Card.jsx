...
function Card({ data }) {
  return (
    <div
      ...
      style={{ boxShadow: "5px 5px 0px 0px red" }}
    >
      <div className="flex-1">
        <a
          href={`https://www.reddit.com${data.permalink}`}
          target="_blank"
          className="text-2xl text-red-500 mb-2 underline font-semibold"
        >
          {data.title}
        </a>
        <h1 className="text-xl my-2 font-mono">Author: u/{data.author}</h1>
        {data.selftext.length <= 1000 && (
          <p className="font-mono">{data.selftext.split(" ").slice(0, 80).join(" ")}</p>
        )}
      </div>
      <div className="flex">
        <img
